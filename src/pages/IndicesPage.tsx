...
export const IndicesPage = () => {
  const [activeTab, setActiveTab] = useState('Cash Indices');

  const cashIndices = [
    { name: 'US 30', fullName: 'Wall Street 30', price: '38,985.50', change: '+0.45%', isNegative: false, icon: '🇺🇸' },
    { name: 'US 500', fullName: 'S&P 500 Index', price: '5,137.08', change: '+0.80%', isNegative: false, icon: '🇺🇸' },
    { name: 'US Tech 100', fullName: 'Nasdaq 100', price: '18,302.90', change: '+1.14%', isNegative: false, icon: '🇺🇸' },
    { name: 'UK 100', fullName: 'FTSE 100 Index', price: '7,640.33', change: '-0.12%', isNegative: true, icon: '🇬🇧' },
    { name: 'Germany 40', fullName: 'DAX 40 Index', price: '17,735.07', change: '+0.32%', isNegative: false, icon: '🇩🇪' },
    { name: 'Japan 225', fullName: 'Nikkei 225', price: '39,910.82', change: '+1.90%', isNegative: false, icon: '🇯🇵' },
  ];

  const futureIndices = [
    { name: 'US 30 Future', fullName: 'Wall Street 30 Future', price: '39,012.00', change: '+0.42%', isNegative: false, icon: '🇺🇸' },
    { name: 'US 500 Future', fullName: 'S&P 500 Future', price: '5,145.50', change: '+0.78%', isNegative: false, icon: '🇺🇸' },
    { name: 'US Tech 100 Future', fullName: 'Nasdaq 100 Future', price: '18,345.25', change: '+1.10%', isNegative: false, icon: '🇺🇸' },
    { name: 'UK 100 Future', fullName: 'FTSE 100 Future', price: '7,652.00', change: '-0.10%', isNegative: true, icon: '🇬🇧' },
    { name: 'Germany 40 Future', fullName: 'DAX 40 Future', price: '17,750.50', change: '+0.30%', isNegative: false, icon: '🇩🇪' },
  ];

  const currentList = activeTab === 'Cash Indices' ? cashIndices : futureIndices;
    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <section className="py-14 lg:py-16 text-center bg-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          >
            Speculate on the performance of the world's leading stock markets. Access major global indices with competitive spreads and flexible leverage.
          </motion.p>
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-center mb-12">
            <div className="bg-gray-100 p-1.5 rounded-full inline-flex">
              {['Cash Indices', 'Future Indices'].map((tab) => (
                <button
                  key={tab}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-50">
                {currentList.map((index) => (
                  <tr key={index.name} className="hover:bg-gray-50/30 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <span className="text-2xl">{index.icon}</span>
                        <span className="font-bold text-gray-900">{index.name}</span>
                      </div>
                    </td>
                    <td className="px-8 py-6 text-gray-500 font-medium">{index.fullName}</td>
                    <td className="px-8 py-6 text-right font-bold text-gray-900">{index.price}</td>
                    <td className={`px-8 py-6 text-right font-bold ${index.isNegative ? 'text-primary' : 'text-green-500'}`}>
                      {index.change}
                    </td>
          ...
          <div className="mt-12 text-center">
            <p className="text-gray-500 mb-6">Want to see more instruments?</p>
      ...
      {/* Why Trade Indices Section */}
      <section className="py-14 lg:py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-12 items-center">
            <div>
              <span className="text-sm font-bold text-primary uppercase tracking-widest mb-4 block">BENEFITS</span>
              <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-8 leading-tight">
                Why Trade Indices with <br /> <span className="text-primary">REVO CAPITAL?</span>
              </h2>
              <div className="space-y-6">
                {[
                  { title: 'Deep Liquidity', desc: 'Execute large orders with minimal slippage on major global indices.' },
                  { title: 'Competitive Spreads', desc: 'Trade with some of the tightest spreads in the industry, starting from 0.4 points.' },
                  { title: 'Flexible Leverage', desc: 'Maximize your market exposure with leverage up to 20:1 on major indices.' },
                  { title: 'No Commissions', desc: 'Trade cash indices with zero commission fees on all account types.' }
                ].map((item, idx) => (
            ...
            <div className="relative">
              <div className="bg-white p-4 rounded-[2.5rem] shadow-2xl relative z-10">
                <img 
                  src="https://picsum.photos/seed/indices-trading/800/600" 
                  alt="Indices Trading" 
        ...
        <div className="max-w-7xl mx-auto px-4 flex flex-col lg:flex-row items-center gap-10 lg:gap-12">
          ...
          <div className="lg:w-1/2 relative">
            <div className="relative z-10 bg-gray-800 p-4 rounded-3xl border border-white/10 shadow-2xl">
              <img src="https://picsum.photos/seed/platform-indices/800/600" alt="Platform" className="rounded-2xl" referrerPolicy="no-referrer" />
            </div>
